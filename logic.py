...
class Logic:
    ...
    def LoadEditor(self, Game, secondparam):
        ...
        Game.GameObjects.buttonList.append(Game.GameObjects.Button((0, 0, 0), Game.GameScreen.screenInfo.objectWidth, Game.GameScreen.screenInfo.objectHeight, Game.GameObjects.builderLayout.colorStartFieldX, Game.GameObjects.builderLayout.colorStartFieldY, Game.GameLogic.SelectColor, "", 2, Game))
        Game.GameObjects.buttonList.append(Game.GameObjects.Button((255, 255, 255), Game.GameScreen.screenInfo.objectWidth, Game.GameScreen.screenInfo.objectHeight, Game.GameObjects.builderLayout.colorStartFieldX, Game.GameObjects.builderLayout.colorStartFieldY + Game.GameScreen.screenInfo.objectHeight, Game.GameLogic.SelectColor, "", 3, Game))
        Game.GameObjects.buttonList.append(Game.GameObjects.Button(Game.backgroundColor, Game.GameScreen.screenInfo.objectWidth, Game.GameScreen.screenInfo.objectHeight, Game.GameObjects.builderLayout.colorStartFieldX, Game.GameObjects.builderLayout.colorStartFieldY + 2 * Game.GameScreen.screenInfo.objectHeight, Game.GameLogic.SelectColor, "", 4, Game))
        for i in range(3):
            for j in range(4):
        ...
        if secondparam != "":
            for i in range(len(Game.GameObjects.levelList)):
                if (Game.GameObjects.levelList[i] == secondparam):
                    ...
                    Game.backgroundColor = f.readline().strip()
                    Game.playerColor = f.readline().strip()
                    Game.ballColor = f.readline().strip()
                    for x in range(12):
                        row = f.readline().strip().split(' ')
                        for y in range(10):
                            Game.GameObjects.level[x][y] = int(row[y])
                    break           
    def SaveLevel(self, Game, secondparam):
        ...
        for x in range(12):
            for y in range(10):
                f.write(str(Game.GameObjects.level[x][y]) + " ")
            f.write("\n")
        f.close()
